...
module.exports = cls => class IdealTreeBuilder extends cls {
  constructor (options) {
    ...
    console.log("incoming build ideal tree option for ignore versions", this[_ignoreVersions])
  ...
  [_avoidRange] (name) {
    if (!this.auditReport && !this.ignoreVersions) {
      return null
    }
    const vuln = this.auditReport.get(name)
    const ignored = this.ignoreVersions
    if (!vuln && !ignored) {
      return null
    }
    return ignored
